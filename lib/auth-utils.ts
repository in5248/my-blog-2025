...
export function parseClerkError(error: any): AuthError {
  // Clerk 오류 객체에서 정보 추출
  const errorCode = error?.errors?.[0]?.code || error?.code || 'unknown_error'
 ...
 */
export function parseHttpError(status: number, responseData?: any): AuthError {
  switch (status) {
    case 401:
      return {
        type: AuthErrorType.UNAUTHORIZED,
        ...errorMessages[AuthErrorType.UNAUTHORIZED]
      }
    case 403:
      return {
        type: AuthErrorType.PERMISSION_DENIED,
        ...errorMessages[AuthErrorType.PERMISSION_DENIED]
      }
    case 404:
      return {
        type: AuthErrorType.USER_NOT_FOUND,
        ...errorMessages[AuthErrorType.USER_NOT_FOUND]
      }
    case 429:
      return {
        type: AuthErrorType.RATE_LIMITED,
        ...errorMessages[AuthErrorType.RATE_LIMITED]
      }
    case 500:
    case 502:
    case 503:
...
export function parseAuthError(error: unknown): AuthError {
  ...
  if (error && typeof error === 'object' && 'status' in error) {
    return parseHttpError((error as any).status, error)
  }
 ...
 */
export const getRetryDelay = (errorType: AuthErrorType, attemptCount: number): number => {
