...
export const mockAuthors: Author[] = [
  {
    ...
    bio: "한신대 재학생으로 웹 기술과 사용자 경험에 관심이 많습니다.",
    socialLinks: {
...
export const mockPosts: BlogPost[] = [
  {
    ...
    coverImage: "/images/posts/cute-cat.jpg",
    content:
  ...
  },
  {
    ...
    excerpt:
      "TypeScript의 고급 타입 시스템과 유틸리티 타입을 활용하여 더욱 안전하고 표현력 있는 코드를 작성하는 방법을 배워봅니다.",
  ...
  },
  {
    ...
    excerpt:
      "React 18의 Concurrent Features를 활용하여 더욱 반응성 높고 사용자 친화적인 애플리케이션을 구축하는 방법을 알아봅니다.",
    author: mockAuthors[0],
  ...
  },
  {
    ...
    content:
      "# JavaScript ES2024 새로운 기능들과 실무 활용 사례\n\n매년 JavaScript에는 새로운 기능들이 추가됩니다. ES2024(ES15)에서도 개발자의 생산성을 높이는 유용한 기능들이 도입되었습니다...",
    excerpt:
      "JavaScript ES2024에서 새롭게 추가된 기능들을 살펴보고, 실무에서 이를 효과적으로 활용하는 방법을 알아봅니다.",
    author: mockAuthors[1],
  ...
  },
  {
    ...
    content:
      "# 웹 성능 최적화 완벽 가이드: Core Web Vitals부터 실전 기법까지\n\n웹 성능은 사용자 경험과 SEO에 직접적인 영향을 미치는 중요한 요소입니다...",
    excerpt:
      "Google의 Core Web Vitals 지표부터 실전에서 바로 적용할 수 있는 웹 성능 최적화 기법까지 종합적으로 다룹니다.",
    author: mockAuthors[0],
  ...
  },
  {
    ...
    content:
      "# CSS Grid vs Flexbox: 언제 무엇을 사용해야 할까?\n\nCSS의 레이아웃 시스템으로 Grid와 Flexbox가 널리 사용되고 있습니다. 하지만 언제 어떤 것을 사용해야 할지 헷갈리는 경우가 많습니다...",
    excerpt:
      "CSS Grid와 Flexbox의 차이점을 이해하고, 상황에 맞는 최적의 레이아웃 방법을 선택하는 가이드를 제공합니다.",
    author: mockAuthors[1],
    category: mockCategories[0],
...
export const mockComments: Comment[] = [
  {
    ...
    content:
      "Next.js 15의 App Router에 대한 설명이 정말 도움이 되었습니다! 특히 서버 컴포넌트와 클라이언트 컴포넌트의 차이점 부분이 이해하기 쉽게 설명되어 있네요.",
  ...
  },
  {
    ...
    content:
      "@김독자 저도 같은 생각입니다. 실제 프로젝트에 바로 적용해볼 수 있는 예제들이 많아서 좋았어요.",
  ...
  },
  {
    ...
    content:
      "좋은 피드백 감사합니다! 다음 포스트에서는 더 실전적인 예제들을 다뤄보겠습니다.",
  ...
  },
  {
    ...
    content:
      "TypeScript의 고급 타입 기능들이 이렇게 활용할 수 있는지 몰랐네요. 특히 조건부 타입 부분이 인상깊었습니다.",
  ...
  },
  {
    ...
    content:
      "React 18의 Concurrent Features 정말 혁신적이네요! 실제 프로젝트에서 성능 향상을 체감할 수 있었습니다.",
...
export function getCommentStats(postId: string): CommentStats {
  ...
  const approvedComments = postComments.filter(
    (c) => c.status === "approved"
  ).length;
  const pendingComments = postComments.filter(
    (c) => c.status === "pending"
  ).length;
  const spamComments = postComments.filter((c) => c.status === "spam").length;
  const todayComments = postComments.filter(
    (c) => c.createdAt >= todayStart
  ).length;
  const weekComments = postComments.filter(
  ...
  const avgCommentLength =
    totalComments > 0
      ? Math.round(
          postComments.reduce((sum, c) => sum + c.content.length, 0) /
            totalComments
        )
      : 0;
  ...
  const replyRatio =
    totalComments > 0 ? Math.round((replyCount / totalComments) * 100) : 0;
...
export function getRelatedPosts(
  ...
  const otherPosts = mockPosts.filter(
...
export function getLatestPosts(
  ...
  return mockPosts
    .filter(
      (post) => post.status === "published" && !excludeIds.includes(post.id)
    )
    .sort((a, b) => b.publishedAt.getTime() - a.publishedAt.getTime())
    .slice(0, limit);
}

// mockCategories를 categories로 재export
export const categories = mockCategories;

/**
 * 슬러그로 카테고리를 찾는 유틸리티 함수
 */
